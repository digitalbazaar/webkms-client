...
export class AsymmetricKey {
  ...
  getAlgorithm({keyDescription}) {
    ...
    const algorithms = new Map([
      ...
      ['P-256', {hex: 0x1200, prefix: 'Dna'}],
      ['P-384', {hex: 0x1201, prefix: '82L'}],
      ['P-521', {hex: 0x1202, prefix: '2J9'}],
    ...
    const multibaseBs58Header = 'z';
    if(!publicKeyMultibase.startsWith(multibaseBs58Header)) {
      return;
    }
    for(const [algorithm, {prefix}] of algorithms) {
      // match on the bs58 prefix for now
      if(publicKeyMultibase.startsWith(multibaseBs58Header + prefix)) {
        return algorithm;
      }
    }
  }
