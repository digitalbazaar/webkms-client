...
export class AsymmetricKey {
  ...
  getAlgorithm({keyDescription}) {
    ...
    if(publicKeyMultibase.startsWith('z')) {
      ...
      base58btc.decode(publicKeyMultibase.substr(1));
      // if the uint8array startsWith the code return true
      const _startsWith = (array, value) => array.indexOf(value) === 0;
      for(const [algorithm, code] of algorithms) {
        if(_startsWith(publicKeyMulticodec, code)) {
          return algorithm;
        }
      }
    }
