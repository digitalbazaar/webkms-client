...
export class Hmac {
  ...
  async sign({data, useCache = true}) {
    ...
    return this._cache.memoize({
      // `:` is not in the base64url alphabet, so it is used as a delimiter
  ...
  async verify({data, signature, useCache = true}) {
    ...
    return this._cache.memoize({
      // `base64url.encode()` will accept a string or a Uint8Array;
      // `:` is not in the base64url alphabet, so it is used as a delimiter
      key: `verify:${base64url.encode(data)}:${base64url.encode(signature)}`,
