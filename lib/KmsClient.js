...
// default key operation client options
const DEFAULT_KEY_OP_HTTP_CLIENT_OPTIONS = {
  retry: {
    // a total of five requests at most will be made per op, the initial and
    // four retries; each will be tried using the default backoff calculated
    // via `0.3 * (2 ** (attemptCount - 1)) * 1000` so at ms intervals of:
    // 300, 600, 1200, 2400 ... for a total of 4500ms wait time after the
    // initial attempt... if the `timeout` expires before this then retries
    // will stop
    limit: 4,
    // key operations all use `get` or `post` METHOD
    methods: ['get', 'post']
  }
};
...
export class KmsClient {
   ...
   *   with every request.
   * @param {object} [options.keyOperationHttpClientOptions] - The HTTP client
   *   options to use with all key operations (this is for operations that use
   *   an existing key (e.g., sign, verify) it is not used when generating
   *   new keys.
   *
   * @returns {KmsClient} The new instance.
   */
  constructor({
    keystoreId, httpsAgent, defaultHeaders, keyOperationHttpClientOptions
  } = {}) {
    if(keystoreId) {
    ...
    this.keyOperationHttpClientOptions = {
      ...DEFAULT_KEY_OP_HTTP_CLIENT_OPTIONS,
      ...keyOperationHttpClientOptions
    };
  }
  ...
  async wrapKey({kekId, unwrappedKey, capability, invocationSigner}) {
    ...
    try {
      ...
      // send request
      const {agent, keyOperationHttpClientOptions} = this;
      const result = await httpClient.post(url, {
        agent, headers, json: operation,
        ...keyOperationHttpClientOptions
      });
  ...
  async _getUncachedKeyDescription({url, capability, invocationSigner}) {
    ...
    try {
      ...
      const result = await httpClient.get(
        url, {agent, headers, ...keyOperationHttpClientOptions});
      return result.data;
    } catch(e) {
      _handleClientError({
